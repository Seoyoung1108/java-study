...
public class RequestHandler extends Thread {
	...
	public void run() {
		try {
			...
			if("GET".equals(tokens[0])) {
				responseStaticRescources(outputStream,tokens[1],tokens[2]);
			} else {
	...
	private void responseStaticRescources(OutputStream os, String url, String protocol) throws IOException {
		...
		String contentType = Files.probeContentType(file.toPath()); //  ex) text/html
	...
	private void response404Error(OutputStream os, String protocol) throws IOException {
		...
		*/
		...
		os.write((protocol+" 404 File Not Found").getBytes( "UTF-8" ));		
		os.write( ("Content-Type:"+contentType+"; charset=utf-8\n").getBytes( "UTF-8" ) );
		os.write( "\n".getBytes() );
		os.write(body);
	}
